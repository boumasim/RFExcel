    ...
    * For xls, xlrd library returns ints as floats, so floats like 5.00 are implicitly cast to int by this library
    ...
    def _wrap_public_result(self, value: Any) -> Any:
        ...
        if isinstance(value, dict) and not isinstance(value, DotDict):
            ...
            return DotDict(
                {str(k): self._wrap_public_result(v) for k, v in dict_val.items()}
            )
    ...
    def load_workbook(self, path: str, read_only: bool = False, **kwargs: Any) -> None:
        ...
        if self._active_workbook:
            self.close()
        self._active_workbook = self._factory.load_workbook(
            path=path, read_only=read_only, **kwargs
        )
    ...
    @overload
    def get_row(
        self, row: int, headers: None = ..., **kwargs: Any
    ) -> list[NativeType]: ...

    @overload
    def get_row(
        self, row: int, headers: dict[str, int] | list[str], **kwargs: Any
    ) -> DotDict: ...

    @keyword("Get Row")  # pyright: ignore[reportUntypedFunctionDecorator]
    def get_row(
        self, row: int, headers: dict[str, int] | list[str] | None = None, **kwargs: Any
    ) -> DotDict | list[NativeType]:
    ...
    def get_cell(self, cell_name: str) -> NativeType:
        ...
        In read-only mode, row can be only read once, meaning you now cannot read A2 and C2 after each other.
    ...
    def append_row(
        self, row_data: dict[str, InsertNativeType], header_row: int = 1
    ) -> None:
    ...
    def append_rows(
        self, rows: list[dict[str, InsertNativeType]], header_row: int = 1
    ) -> None:
    ...
    def insert_row(
        self, row_data: dict[str, InsertNativeType], row: int, header_row: int = 1
    ) -> None:
    ...
    def compare_data_to(
        ...
        if same_file and not wb.read_only:
            ...
            target = self._factory.load_workbook(
                path=str(target_path_p), read_only=True
            )
