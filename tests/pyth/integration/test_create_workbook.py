...
class TestCreateWorkbookPositive:
    ...
    def test_create_xlsx_sets_active_workbook(self, lib, tmp_path):
        path = str(tmp_path / "new.xlsx")
        lib.create_workbook(path)
        assert lib._active_workbook is not None

    def test_create_xlsx_produces_file_on_disk(self, lib, tmp_path):
        path = tmp_path / "new.xlsx"
        lib.create_workbook(str(path))
        assert path.exists()

    def test_create_csv_sets_active_workbook(self, lib, tmp_path):
        path = str(tmp_path / "new.csv")
        lib.create_workbook(path)
        assert lib._active_workbook is not None

    def test_create_csv_produces_file_on_disk(self, lib, tmp_path):
        path = tmp_path / "new.csv"
        lib.create_workbook(str(path))
        assert path.exists()

    def test_created_xlsx_is_immediately_readable(self, lib, tmp_path):
    ...
    def test_created_csv_is_immediately_readable(self, lib, tmp_path):
...
class TestCreateWorkbookNegative:
    ...
    def test_create_on_existing_xlsx_raises(self, lib, tmp_path):
    ...
    def test_create_on_existing_csv_raises(self, lib, tmp_path):
    ...
    def test_create_xls_raises_format_not_supported(self, lib, tmp_path):
    ...
    def test_create_txt_raises_format_not_supported(self, lib, tmp_path):
        path = str(tmp_path / "notes.txt")
        with pytest.raises(FileFormatNotSupportedException):
            lib.create_workbook(path)

    def test_create_ods_raises_format_not_supported(self, lib, tmp_path):
        path = str(tmp_path / "sheet.ods")
        with pytest.raises(FileFormatNotSupportedException):
            lib.create_workbook(path)

    def test_active_workbook_unchanged_after_failed_create(self, lib, tmp_path):
...
class TestCreateWorkbookEdge:
    ...
    def test_create_xlsx_with_nested_new_directories(self, lib, tmp_path):
        """Parent directories that do not exist must be created automatically."""
        path = str(tmp_path / "a" / "b" / "c" / "deep.xlsx")
        lib.create_workbook(path)
        assert Path(path).exists()

    def test_created_xlsx_can_be_loaded_afterwards(self, lib, tmp_path):
    ...
    def test_created_csv_can_be_loaded_afterwards(self, lib, tmp_path):
    ...
    def test_two_different_workbooks_created_independently(self, lib, tmp_path):
