...
class TestCloseWorkbookPositive:
    ...
    def test_close_after_load_xlsx_sets_active_workbook_to_none(self, lib: RFExcelLibrary):
        lib.load_workbook(XLSX_FILE)
        lib.close()
        assert lib._active_workbook is None

    def test_close_after_load_xlsx_stream_sets_active_workbook_to_none(self, lib: RFExcelLibrary):
        lib.load_workbook(XLSX_FILE, read_only=True)
        lib.close()
        assert lib._active_workbook is None

    def test_close_after_load_xls_sets_active_workbook_to_none(self, lib: RFExcelLibrary):
        lib.load_workbook(XLS_FILE)
        lib.close()
        assert lib._active_workbook is None

    def test_close_after_load_csv_sets_active_workbook_to_none(self, lib: RFExcelLibrary):
        lib.load_workbook(CSV_FILE)
        lib.close()
        assert lib._active_workbook is None

    def test_close_after_create_xlsx_sets_active_workbook_to_none(self, lib: RFExcelLibrary, tmp_path):
        lib.create_workbook(str(tmp_path / "new.xlsx"))
        lib.close()
        assert lib._active_workbook is None

    def test_close_after_create_csv_sets_active_workbook_to_none(self, lib: RFExcelLibrary, tmp_path):
...
class TestCloseWorkbookEdge:
    ...
    def test_close_csv_stream_closes_file_handle(self, lib: RFExcelLibrary):
