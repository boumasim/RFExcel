...
class TestLoadWorkbookPositive:
    ...
    def test_load_xlsx_edit_mode_sets_active_workbook(self, lib):
        lib.load_workbook(XLSX_FILE)
        assert lib._active_workbook is not None

    def test_load_xlsx_stream_mode_sets_active_workbook(self, lib):
        lib.load_workbook(XLSX_FILE, read_only=True)
        assert lib._active_workbook is not None

    def test_load_xls_edit_mode_sets_active_workbook(self, lib):
        lib.load_workbook(XLS_FILE)
        assert lib._active_workbook is not None

    def test_load_xls_on_demand_mode_sets_active_workbook(self, lib):
        lib.load_workbook(XLS_FILE, read_only=True)
        assert lib._active_workbook is not None

    def test_load_csv_edit_mode_sets_active_workbook(self, lib):
        lib.load_workbook(CSV_FILE)
        assert lib._active_workbook is not None

    def test_load_csv_stream_mode_sets_active_workbook(self, lib):
        lib.load_workbook(CSV_FILE, read_only=True)
        assert lib._active_workbook is not None

    def test_load_xlsx_edit_is_immediately_readable(self, lib):
        lib.load_workbook(XLSX_FILE)
        rows = lib.get_rows()
        assert len(rows) == 4
        assert rows[0]["Product ID"] == "P-200"

    def test_load_xlsx_stream_is_immediately_readable(self, lib):
        lib.load_workbook(XLSX_FILE, read_only=True)
        rows = lib.get_rows()
        assert len(rows) == 4

    def test_load_xls_edit_is_immediately_readable(self, lib):
        lib.load_workbook(XLS_FILE)
        rows = lib.get_rows()
        assert len(rows) == 9

    def test_load_xls_on_demand_is_immediately_readable(self, lib):
        lib.load_workbook(XLS_FILE, read_only=True)
        rows = lib.get_rows()
        assert len(rows) == 9

    def test_load_csv_edit_is_immediately_readable(self, lib):
        lib.load_workbook(CSV_FILE)
        rows = lib.get_rows()
        assert len(rows) == 4

    def test_load_csv_stream_is_immediately_readable(self, lib):
        lib.load_workbook(CSV_FILE, read_only=True)
        rows = lib.get_rows()
        assert len(rows) == 4
...
class TestLoadWorkbookEdge:
    ...
    def test_loading_second_file_replaces_first(self, lib):
    ...
    def test_loading_after_close_works(self, lib):
        lib.load_workbook(XLSX_FILE)
        lib.close()
        lib.load_workbook(XLSX_FILE)
        assert lib._active_workbook is not None
    ...
    def test_xlsx_edit_and_stream_produce_identical_rows(self, lib):
        lib.load_workbook(XLSX_FILE)
    ...
    def test_csv_edit_and_stream_produce_identical_rows(self, lib):
        lib.load_workbook(CSV_FILE)
        edit_rows = lib.get_rows()
        lib.close()
        lib.load_workbook(CSV_FILE, read_only=True)
    ...
    def test_xls_edit_and_on_demand_produce_identical_rows(self, lib):
        lib.load_workbook(XLS_FILE)
        edit_rows = lib.get_rows()
        lib.close()
        lib.load_workbook(XLS_FILE, read_only=True)
